  ...
  export default function ServicesPage() {
    ...
    // ✅ Optional: refresh when switching to "list" tab
    useEffect(() => {
      if (activeTab === 'list') {
        fetchServices();
      }
    ...
    const validateForm = (): boolean => {
      ...
      if (formData.deliery_time_days === '') {
        newErrors.deliery_time_days = 'Delivery time is required';
      } else if (formData.deliery_time_days < 1) {
        newErrors.deliery_time_days = 'Delivery time must be at least 1 day';
      }

      if (formData.image_urls.length === 0 && imagePreviews.length === 0) {
    ...
    const handleInputChange = (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
      ...
      setFormData(prev => ({
        ...prev,
        [name]: name === 'service_price' || name === 'deliery_time_days' ? (value === '' ? '' : Number(value)) : value
      }));
    ...
    const handleImageUpload = (e: ChangeEvent<HTMLInputElement>) => {
      ...
      if (files.length + formData.image_urls.length + imagePreviews.length > 10) {
        alert('Maximum 10 image_urls allowed');
        return;
      }
    ...
    const handleEditService = (service: Service) => {
      ...
      setFormData({
        ...
        deliery_time_days: service.deliery_time_days,
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/add-service`, {
        ...
        body: JSON.stringify({
          ...
          delivery_time_days: formData.deliery_time_days,
      ...
      const newService: Service = {
        service_id: data.registeredService.service_id,
        service_name: data.registeredService.service_name,
        service_price: data.registeredService.service_price,
        description: data.registeredService.description,
        deliery_time_days: data.registeredService.delivery_time_days,
        is_active: true,
        image_urls: data.registeredService.image_urls,
        createdAt: data.registeredService.created_at,
      };
    ...
    const handleToggleServiceStatus = (service_id: string) => {
                          ...
                          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
                            ...
                            <div className="flex items-center space-x-6 text-gray-500">
                              <div className="flex items-center">
                                ...
                                <span className="font-medium">{service.deliery_time_days} days delivery</span>
                              </div>
            ...
            <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-4xl mx-auto">
              <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    <label htmlFor="deliery_time_days" className="flex items-center text-lg font-semibold text-gray-800 mb-3">
                      <Clock className="w-5 h-5 mr-2 text-purple-600" />
                      Delivery Time (Days) *
                    </label>
                    <div className="relative">
                      <input
                        id="deliery_time_days"
                        name="deliery_time_days"
                        type="number"
                        min="1"
                        required
                        value={formData.deliery_time_days}
                        onChange={handleInputChange}
                        className={`w-full pl-10 pr-4 py-3 border-2 rounded-xl shadow-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 text-base transition-all duration-200 ${
                          errors.deliery_time_days 
                            ? 'border-red-300 focus:border-red-500 focus:ring-red-500' 
                            : 'border-gray-200 hover:border-gray-300 focus:border-purple-500'
                        }`}
                        placeholder="e.g., 7"
                ...
                <div>
                  ...
                  >
                    ...
                    <h3 className="text-lg font-semibold text-gray-700 mb-2">
                      Upload Service image_urls
                    </h3>
                    <p className="text-gray-500 text-sm">
                      Click to upload or drag and drop (Max 10 image_urls, 10MB each)
                    </p>
                  </div>
                  ...
                  {(imagePreviews.length > 0 || formData.image_urls.length > 0) && (
                    <div className="mt-6">
                      <h4 className="text-sm font-semibold text-gray-700 mb-3">
                        Uploaded image_urls ({imagePreviews.length + formData.image_urls.length}/10)
                      </h4>
